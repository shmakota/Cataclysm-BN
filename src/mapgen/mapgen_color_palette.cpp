...
auto MapgenColorPalette::get_unique_id() -> mpalette_id {
    ...
    while (true) {
        ...
        if (!new_group.is_valid()) { return new_group; }
    }
}
void MapgenColorPalette::load(const JsonObject& jo, const std::string&) {
    if (jo.has_bool("clear") && jo.get_bool("clear")) { colors.clear(); }
...
void MapgenColorPalette::check_definitions() { all_palettes.check(); }
void MapgenColorPalette::check() const {
    for (auto colorstr : colors) {
        std::optional<RGBColor> color = RGBColor::try_parse(colorstr.obj);
        if (!color) { debugmsg("Invalid Color %s in Mapgen Palette %s", colorstr.obj, id.str()); }
    }
}
...
void MapgenColorPalette::reset() { all_palettes.reset(); }
