...
mapbuffer_registry::mapbuffer_registry() {
    ...
    auto& primary =
        *buffers_.emplace(primary_dimension_id(), std::make_unique<mapbuffer>()).first->second;
...
auto mapbuffer_registry::unload_dimension(const dimension_id& dim_id) -> void {
    if (buffers_.erase(dim_id) > 0) { ++generation_; }
}

auto mapbuffer_registry::for_each(const std::function<void(const dimension_id&, mapbuffer&)>& fn)
    -> void {
    for (auto& kv : buffers_) { fn(kv.first, *kv.second); }
}

auto mapbuffer_registry::primary() -> mapbuffer& { return get(primary_dimension_id()); }

auto mapbuffer_registry::active() -> mapbuffer& { return get(g_active_dimension_id); }
...
auto mapbuffer_registry::active_dimension_ids() const -> std::vector<dimension_id> {
    ...
    for (const auto& kv : buffers_) { ids.push_back(kv.first); }
    return ids;
}
