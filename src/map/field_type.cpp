...
template <> std::string enum_to_string<description_affix>(description_affix data) {
    switch (data) {
        ...
        case description_affix::DESCRIPTION_AFFIX_COVERED_IN:
            return "covered_in";
        case description_affix::DESCRIPTION_AFFIX_COVERED_IN_A:
            return "covered_in_a";
        case description_affix::DESCRIPTION_AFFIX_ON:
            return "on";
        case description_affix::DESCRIPTION_AFFIX_UNDER:
            return "under";
        case description_affix::DESCRIPTION_AFFIX_ILLUMINTED_BY:
...
template <> std::string enum_to_string<fields::stacking_type>(fields::stacking_type data) {
    switch (data) {
        ...
        case fields::stacking_type::intensity:
            return "intensity";
        case fields::stacking_type::duration:
            return "duration";
        // *INDENT-ON*
...
void field_type::load(const JsonObject& jo, const std::string&) {
    ...
    for (const JsonObject jao : jo.get_array("intensity_levels")) {
        ...
        field_intensity_level fallback_intensity_level =
            !intensity_levels.empty() ? intensity_levels.back() : intensity_level;
        optional(jao, was_loaded, "name", intensity_level.name, fallback_intensity_level.name);
        optional(jao, was_loaded, "sym", intensity_level.symbol,
                 unicode_codepoint_from_symbol_reader, fallback_intensity_level.symbol);
        intensity_level.color =
            jao.has_member("color")
                ? color_from_string(jao.get_string("color"))
        ...
        optional(jao, was_loaded, "radiation_hurt_damage_min",
                 intensity_level.radiation_hurt_damage_min,
                 fallback_intensity_level.radiation_hurt_damage_min);
        optional(jao, was_loaded, "radiation_hurt_damage_max",
                 intensity_level.radiation_hurt_damage_max,
        ...
        optional(jao, was_loaded, "intensity_upgrade_chance",
                 intensity_level.intensity_upgrade_chance,
                 fallback_intensity_level.intensity_upgrade_chance);
        optional(jao, was_loaded, "intensity_upgrade_duration",
                 intensity_level.intensity_upgrade_duration,
        ...
        optional(jao, was_loaded, "convection_temperature_mod",
                 intensity_level.convection_temperature_mod,
        ...
        if (jao.has_array("effects")) {
            for (const JsonObject joe : jao.get_array("effects")) {
                ...
                const auto game_message_type_reader = enum_flags_reader<game_message_type>{
                    "game message types"};
                optional(joe, was_loaded, "message_type", fe.env_message_type,
                         game_message_type_reader);
    ...
    for (const std::string id : jid.get_array("traits")) { immunity_data_traits.emplace_back(id); }
    for (JsonArray jao : jid.get_array("body_part_env_resistance")) {
        immunity_data_body_part_env_resistance
            .emplace_back(get_body_part_token(jao.get_string(0)), jao.get_int(1));
    }
    ...
    const auto description_affix_reader = enum_flags_reader<description_affix>{
        "description affixes"};
    optional(jo, was_loaded, "description_affix", desc_affix, description_affix_reader,
             description_affix::DESCRIPTION_AFFIX_IN);
    if (jo.has_member("phase")) { phase = jo.get_enum_value<phase_id>("phase", PNULL); }
    const auto stacking_type_reader = enum_flags_reader<fields::stacking_type>{
...
void field_type::finalize() {
    ...
    for (const mtype_id& m_id : immune_mtypes) {
        if (!m_id.is_valid()) {
            debugmsg("Invalid mtype_id %s in immune_mtypes for field %s.", m_id.c_str(),
                     id.c_str());
        }
    }
}
...
void field_type::check() const {
    ...
    for (auto& intensity_level : intensity_levels) {
        ...
        if (intensity_level.name.empty()) {
            debugmsg("Intensity level %d defined for field type \"%s\" has empty name.", i,
                     id.c_str());
        }
    }
...
void field_types::check_consistency() { all_field_types.check(); }
...
field_type_id fd_null, fd_blood, fd_bile, fd_gibs_flesh, fd_gibs_veggy, fd_web, fd_slime, fd_acid,
    ...
    fd_fire_vent, fd_flame_burst, fd_electricity, fd_fatigue, fd_push_items, fd_shock_vent,
    fd_acid_vent, fd_plasma, fd_laser, fd_spotlight, fd_dazzling, fd_blood_veggy, fd_blood_insect,
