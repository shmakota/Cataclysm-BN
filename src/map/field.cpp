...
int field_entry::move_cost() const { return type.obj().get_move_cost(intensity - 1); }
...
float field_entry::light_emitted() const { return type.obj().get_light_emitted(intensity - 1); }

float field_entry::local_light_override() const {
    return type.obj().get_local_light_override(intensity - 1);
}

float field_entry::translucency() const { return type.obj().get_translucency(intensity - 1); }

bool field_entry::is_transparent() const { return type.obj().get_transparent(intensity - 1); }

int field_entry::convection_temperature_mod() const {
    return type.obj().get_convection_temperature_mod(intensity - 1);
}

nc_color field_entry::color() const { return type.obj().get_color(intensity - 1); }

std::string field_entry::symbol() const { return type.obj().get_symbol(intensity - 1); }

field_type_id field_entry::get_field_type() const { return type; }
...
int field_entry::get_max_field_intensity() const { return type.obj().get_max_intensity(); }

int field_entry::get_field_intensity() const { return intensity; }
...
time_duration field_entry::get_field_age() const { return age; }

time_duration field_entry::set_field_age(const time_duration& new_age) { return age = new_age; }

field::field(): _displayed_field_type(fd_null) {}

/*
Function: find_field
Returns a field entry corresponding to the field_type_id parameter passed in. If no fields are found
then returns NULL. Good for checking for existence of a field: if(myfield.find_field(fd_fire)) would
tell you if the field is on fire.
*/
field_entry* field::find_field(const field_type_id& field_type_to_find) {
    if (!_displayed_field_type) { return nullptr; }
    const auto it = _field_type_list.find(field_type_to_find);
    if (it != _field_type_list.end()) { return &it->second; }
    return nullptr;
}
...
If the field already exists, it will return false BUT it will add the intensity/age to the current
values for upkeep. If you wish to modify an already existing field use find_field and modify the
result. Intensity defaults to 1, and age to 0 (permanent) if not specified.
*/
bool field::add_field(
    const field_type_id& field_type_to_add, const int new_intensity, const time_duration& new_age) {
    ...
    if (it != _field_type_list.end()) {
        ...
        if (it->first->stacking_type == fields::stacking_type::intensity) {
            ...
            if (new_age < half_life) { it->second.mod_field_age(new_age - half_life); }
        }
        return false;
    }
...
bool field::remove_field(const field_type_id& field_to_remove) {
    ...
    if (it == _field_type_list.end()) { return false; }
    remove_field(it);
    return true;
}
...
void field::remove_field(std::map<field_type_id, field_entry>::iterator const it) {
    ...
    if (!_field_type_list.empty()) {
        for (auto& fld : _field_type_list) {
            if (!_displayed_field_type
                || fld.first.obj().priority >= _displayed_field_type.obj().priority) {
...
*/
unsigned int field::field_count() const { return _field_type_list.size(); }

std::map<field_type_id, field_entry>::iterator field::begin() { return _field_type_list.begin(); }

std::map<field_type_id, field_entry>::const_iterator field::begin() const {
    return _field_type_list.begin();
}

std::map<field_type_id, field_entry>::iterator field::end() { return _field_type_list.end(); }
...
*/
field_type_id field::displayed_field_type() const { return _displayed_field_type; }
...
int field::total_move_cost() const {
    int current_cost = 0;
    for (auto& fld : _field_type_list) { current_cost += fld.second.move_cost(); }
    return current_cost;
}
