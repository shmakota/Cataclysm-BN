...
enum astar_state { ASL_NONE, ASL_OPEN, ASL_CLOSED };
    ...
    void init(point_abs_ms min, point_abs_ms max) {
        std::ranges::for_each(
            cata::views::cartesian_product(
                std::views::iota(min.x(), max.x() + 1), std::views::iota(min.y(), max.y() + 1)),
            [this](auto xy) {
                const auto& [x, y] = xy;
...
struct pathfinder {
    ...
    std::priority_queue<
        std::pair<int, tripoint_abs_ms>, std::vector<std::pair<int, tripoint_abs_ms>>,
        pair_greater_cmp_first>
        open;
    ...
    void add_point(
        const int gscore, const int score, const tripoint_abs_ms& from, const tripoint_abs_ms& to) {
...
auto get_pf_special_from_tile(const legacy_pathfinding_tile& tile) -> pf_special {
    ...
    if (tile.vehicle_ptr() != nullptr) { cur_value |= PF_VEHICLE; }

    if (!tile.trap.obj().is_benign() || !terrain.trap.obj().is_benign()) { cur_value |= PF_TRAP; }
...
auto map::route(
    ...
    if (f == t || !inbounds(abs_to_map_local(*this, f))) { return ret; }

    if (!inbounds(abs_to_map_local(*this, t))) { return ret; }
    ...
    if (f.z() == t.z()) {
        ...
        if (!(get_pf_special_abs(*this, f) & non_normal)
            ...
            if (is_disjoint(sorted_line, pre_closed)) { return line_path; }
        }
    }
    ...
    if (rl_dist(f, t) > std::min(settings.max_dist, dist_cap)) { return ret; }
    ...
    auto min = tripoint_abs_ms(
        std::min(f.x(), t.x()) - pad, std::min(f.y(), t.y()) - pad, std::min(f.z(), t.z()));
    auto max = tripoint_abs_ms(
        std::max(f.x(), t.x()) + pad, std::max(f.y(), t.y()) + pad, std::max(f.z(), t.z()));
    ...
    do {
        ...
        if (cur_state == ASL_CLOSED) { continue; }
        ...
        for (size_t i = 0; i < 8; i++) {
            ...
            if (layer.state[index] == ASL_CLOSED) { continue; }
            ...
            if (cur_veh
                && !cur_veh->allowed_move(cur_veh->abs_to_mount(cur), cur_veh->abs_to_mount(p))) {
                // Trying to squeeze through a vehicle hole, skip this movement but don't close the
                // tile as other paths may lead to it
                continue;
            }

            if (veh && veh != cur_veh
                && !veh->allowed_move(veh->abs_to_mount(cur), veh->abs_to_mount(p))) {
            ...
            int newg =
                layer.gscore[parent_index] + ((cur.x() != p.x() && cur.y() != p.y()) ? 1 : 0);
            ...
            if (!(p_special & non_normal)) {
                ...
                if (cost == 0 && rating <= 0 && (!doors || !terrain.open || !furniture.open)
                    && veh == nullptr && climb_cost <= 0) {
                    layer.state[index] = ASL_CLOSED; // Close it so that next time we won't try to
                                                     // calculate costs
                    continue;
                }
                ...
                if (cost == 0) {
                    if (climb_cost > 0 && p_special & PF_CLIMBABLE) {
                        ...
                        doors && (terrain.open || furniture.open)
                        && (!terrain.has_flag("OPENCLOSE_INSIDE")
                        ...
                        const auto vpobst =
                            vpart_position(const_cast<vehicle&>(*veh), part).obstacle_at_part();
                        part = vpobst ? vpobst->part_index() : -1;
                        if (doors && veh->part_flag(part, VPFLAG_OPENABLE)
                            && (!veh->part_flag(part, "OPENCLOSE_INSIDE") || cur_veh == veh)) {
                            ...
                            if (hp / 20 > bash) {
                                ...
                                // Threshold damage thing means we will fail to deal damage pretty
                                // often
                                hp *= 2;
                            }
                    ...
                    if (!trp.is_benign()) {
                        ...
                        if (terrain.has_flag(TFLAG_NO_FLOOR)) {
                            ...
                            if (buffer.valid_move(p, below, {.flying = true})) {
                                ...
                                if (below_tile
                                    && !below_tile->get_ter_t().has_flag(TFLAG_NO_FLOOR)) {
                                    // Otherwise this would have been a huge fall
                                    auto& layer = pf.get_layer(p.z() - 1);
                                    // From cur, not p, because we won't be walking on air
                                    pf.add_point(
                                        layer.gscore[parent_index] + 10,
                                        layer.score[parent_index] + 10 + 2 * rl_dist(below, t), cur,
                                        below);
                                }
        ...
        if (settings.allow_climb_stairs && cur.z() > min.z()
            ...
            if (vertical_move_destination<TFLAG_GOES_UP>(*this, dest)) {
                ...
                pf.add_point(layer.gscore[parent_index] + 2,
                             layer.score[parent_index] + 2 * rl_dist(dest, t), cur, dest);
            }
        }
        if (settings.allow_climb_stairs && cur.z() < max.z()
            && parent_terrain.has_flag(TFLAG_GOES_UP)) {
            tripoint_abs_ms dest(cur.xy(), cur.z() + 1);
        ...
        if (cur.z() < max.z() && parent_terrain.has_flag(TFLAG_RAMP)
            && buffer.valid_move(cur, cur + tripoint_rel_ms::above(), {.flying = true})) {
            auto& layer = pf.get_layer(cur.z() + 1);
            for (size_t it = 0; it < 8; it++) {
                const tripoint_abs_ms
                    above(cur.x() + x_offset[it], cur.y() + y_offset[it], cur.z() + 1);
                pf.add_point(layer.gscore[parent_index] + 4,
                             layer.score[parent_index] + 4 + 2 * rl_dist(above, t), cur, above);
            }
        }
        if (cur.z() < max.z() && parent_terrain.has_flag(TFLAG_RAMP_UP)
            && buffer.valid_move(
                cur, cur + tripoint_rel_ms::above(), {.flying = true, .via_ramp = true})) {
            auto& layer = pf.get_layer(cur.z() + 1);
            for (size_t it = 0; it < 8; it++) {
                const tripoint_abs_ms
                    above(cur.x() + x_offset[it], cur.y() + y_offset[it], cur.z() + 1);
        ...
        if (cur.z() > min.z() && parent_terrain.has_flag(TFLAG_RAMP_DOWN)
            && buffer.valid_move(
                cur, cur + tripoint_rel_ms::below(), {.flying = true, .via_ramp = true})) {
            auto& layer = pf.get_layer(cur.z() - 1);
            for (size_t it = 0; it < 8; it++) {
                const tripoint_abs_ms
                    below(cur.x() + x_offset[it], cur.y() + y_offset[it], cur.z() - 1);
                pf.add_point(layer.gscore[parent_index] + 4,
                             layer.score[parent_index] + 4 + 2 * rl_dist(below, t), cur, below);
            }
        }
    ...
    if (done) {
        ...
        for (int fdist = max_length; fdist != 0; fdist--) {
            ...
            if (rl_dist(cur, par) > 1 && std::abs(cur.z() - par.z()) != 1) {
                debugmsg("Jump in our route!  %d:%d:%d->%d:%d:%d", cur.x(), cur.y(), cur.z(),
                         par.x(), par.y(), par.z());
                return ret;
            }
...
auto map::route(
    ...
    const auto abs_route =
        route(map_local_to_abs(*this, clipped_f), map_local_to_abs(*this, clipped_t), settings,
              pre_closed_abs);
    std::vector<tripoint_bub_ms> result;
    result.reserve(abs_route.size());
    for (const tripoint_abs_ms& p : abs_route) { result.push_back(abs_to_map_local(*this, p)); }
    return result;
}
