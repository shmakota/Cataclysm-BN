...
struct vehicle_external_light_cache_scope {
    ...
    ~vehicle_external_light_cache_scope() {
        ...
        for (const auto& [idx, value] : lm_restore) { cache.lm[idx] = value; }
        for (const auto& [idx, value] : sm_restore) { cache.sm[idx] = value; }
    }
};
...
struct ambient_cache_key {
    ...
    friend auto operator==(const ambient_cache_key& lhs, const ambient_cache_key& rhs) -> bool {
        return lhs.owner == rhs.owner && lhs.x == rhs.x && lhs.y == rhs.y && lhs.z == rhs.z
            && lhs.turn == rhs.turn && lhs.generation == rhs.generation;
    }
};
...
struct ambient_cache_key_hash {
    auto operator()(const ambient_cache_key& key) const -> std::size_t {
        ...
        seed ^= static_cast<std::size_t>(key.x) + 0x9e3779b97f4a7c15ull + (seed << 6) + (seed >> 2);
        ...
        seed ^=
            static_cast<std::size_t>(key.turn) + 0x9e3779b97f4a7c15ull + (seed << 6) + (seed >> 2);
        seed ^= static_cast<std::size_t>(key.generation) + 0x9e3779b97f4a7c15ull + (seed << 6)
              + (seed >> 2);
        return seed;
    }
};
...
struct ambient_read_location_key {
    ...
    friend auto operator==(
        const ambient_read_location_key& lhs, const ambient_read_location_key& rhs) -> bool {
...
struct ambient_read_location_hash {
    auto operator()(const ambient_read_location_key& key) const -> std::size_t {
        ...
        seed ^=
            static_cast<std::size_t>(key.line) + 0x9e3779b97f4a7c15ull + (seed << 6) + (seed >> 2);
        return seed;
    }
};
...
std::unordered_map<
    ambient_read_location_key, ambient_read_location_counts, ambient_read_location_hash>
...
auto ambient_read_plot_name(const std::source_location& location, const std::string_view suffix)
    -> std::string {
...
void record_cpu_lm_read(
    const bool valid, std::atomic<int64_t>& valid_counter, std::atomic<int64_t>& stale_counter) {
...
auto packed_colored_light_value(const float intensity, const uint32_t color_rgb) -> uint32_t {
    if (intensity <= LIGHT_AMBIENT_LOW || color_rgb == 0u) { return 0u; }
...
auto atomic_colored_light_max(uint32_t& cell, const uint32_t value) -> void {
    ...
    while (
        ...
        && !__atomic_compare_exchange_n(
            &cell, &expected, value, true, __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {}
#endif
}

auto write_colored_light_cache(
    level_cache& cache, const int idx, const float intensity, const uint32_t color_rgb) -> void {
    atomic_colored_light_max(
        cache.colored_light_cache[idx], packed_colored_light_value(intensity, color_rgb));
}

auto cpu_colored_light_allows_direction(
    const cpu_colored_light_3d_context& ctx, const int x, const int y) -> bool {
    ...
    if (dist_xy_sq <= 0.0001f) { return false; }
    const auto cone_dot = (dx * ctx.dir_x + dy * ctx.dir_y) / std::sqrt(dist_xy_sq);
    return cone_dot >= ctx.cone_cos;
}

auto update_cpu_colored_light_cache(
    void* context, const int, const int, const int, const int idx, const float intensity, quadrant)
    -> void {
    auto& ctx = *static_cast<cpu_colored_light_cache_context*>(context);
    if (ctx.cache == nullptr) { return; }
    write_colored_light_cache(*ctx.cache, idx, intensity, ctx.color_rgb);
}

auto update_cpu_colored_light_cache_3d(
    void* context, const int z_index, const int x, const int y, const int idx,
    const float intensity, quadrant) -> void {
    auto& ctx = *static_cast<cpu_colored_light_3d_context*>(context);
    if (ctx.here == nullptr || intensity <= LIGHT_AMBIENT_LOW) { return; }
    const auto zlev = z_index - OVERMAP_DEPTH;
    if (zlev == ctx.skip_zlev || !cpu_colored_light_allows_direction(ctx, x, y)) { return; }
    ...
    if (distance > 0.5f) {
        adjusted_intensity = std::
            min(adjusted_intensity,
                ctx.luminance / (std::exp(LIGHT_TRANSPARENCY_OPEN_AIR * distance) * distance));
    }
    if (adjusted_intensity <= LIGHT_AMBIENT_LOW) { return; }
    auto& cache = ctx.here->access_cache(zlev);
    write_colored_light_cache(cache, idx, adjusted_intensity, ctx.color_rgb);
}

auto make_colored_light_callback(
    level_cache& cache, const uint32_t color_rgb, cpu_colored_light_cache_context& context)
    -> light_update_callback {
    if (!colored_lighting || color_rgb == 0u) { return {}; }
...
void map::add_light_from_items(
    const tripoint_bub_ms& p, const item_stack::iterator& begin, const item_stack::iterator& end) {
...
auto map::build_transparency_cache(const int zlev) -> bool {
    ...
    if (map_cache.transparency_cache_dirty.none()) { return false; }
    ...
    if (cata_compute::uses_sdl_gpu_compute()) {
        ...
        for (const auto smx : std::views::iota(0, my_MAPSIZE)) {
            for (const auto smy : std::views::iota(0, my_MAPSIZE)) {
                if (!rebuild_all
                    && !map_cache.transparency_cache_dirty.test(
                ...
                if (cur_submap == nullptr) {
                    ...
                    if (!rebuild_all) {
                        for (const auto sx : std::views::iota(0, SEEX)) {
                            std::fill_n(
                                transparency_cache.data()
                                    + map_cache.idx(sm_offset.x() + sx, sm_offset.y()),
        ...
        const auto resident_level_was_valid = cata_gpu::lighting_transparency_level_is_valid(zlev);
        ...
        for (const auto& ref : refs) {
            ...
            for (const auto sm_ms : submap_tiles()) {
                ...
                const auto compact_idx =
                    ref_index * static_cast<size_t>(SEEX * SEEY)
...
#if defined(CATA_SDL) && defined(CATA_GPU_VERIFY)
    cata_gpu::verify_transparency_against_cpu(*this, zlev, get_weather().weather_id->sight_penalty);
#endif
...
auto map::build_transparency_caches(const int minz, const int maxz) -> std::vector<int> {
    ...
    if (cata_compute::uses_sdl_gpu_compute()) {
        ...
        for (const auto zlev : std::views::iota(minz, maxz + 1)) {
            ...
            if (map_cache.transparency_cache_dirty.none()) { continue; }
            ...
            refs.reserve(
                refs.size()
                + static_cast<size_t>(map_cache.cache_mapsize * map_cache.cache_mapsize));
            for (const auto smx : std::views::iota(0, my_MAPSIZE)) {
                for (const auto smy : std::views::iota(0, my_MAPSIZE)) {
                    if (!rebuild_all
                        && !map_cache.transparency_cache_dirty.test(
        ...
        if (dirty_levels.empty()) { return dirty_levels; }
        if (refs.empty()) {
        ...
        if (gpu_result.size() != expected_compact_result_size) {
            debugmsg(
                "SDL_GPU batched transparency dispatch returned %zu compact values, expected %zu",
                gpu_result.size(), expected_compact_result_size);
            return dirty_levels;
        }
        ...
        for (const auto& state : level_states) {
            ...
            if (state.rebuild_all
                    ? state.resident_output_complete
                    : state.resident_level_was_valid) {
...
#endif
    for (const auto zlev : std::views::iota(minz, maxz + 1)) {
        if (build_transparency_cache(zlev)) { dirty_levels.push_back(zlev); }
    }
    return dirty_levels;
}
...
auto map::build_vision_transparency_cache(const Character& player) -> bool {
    ...
    if (player.movement_mode_is(CMM_CROUCH) || player.movement_mode_is(CMM_PRONE)) {
        ...
        const auto check_vehicle_coverage =
            [](const vehicle* veh, const tripoint_mnt_veh& p) -> bool {
            return veh->obstacle_at_position(p) == -1
                && (veh->part_with_feature(p, "AISLE", true) != -1
                    || veh->part_with_feature(p, "PROTRUSION", true) != -1);
        };

        const optional_vpart_position player_vp = veh_at(p);

        tripoint_mnt_veh player_mount;
        if (player_vp) { player_mount = player_vp->vehicle().bubble_to_mount(tripoint_bub_ms(p)); }
        ...
        for (point adjacent : eight_adjacent_offsets) {
            ...
            if (coverage(p + adjacent) >= 30) {
                ...
                if (std::ranges::find(four_diagonal_offsets, adjacent)
                    != four_diagonal_offsets.end()) {
...
void map::apply_character_light(Character& p) {
    ...
    const auto colored_light =
        colored_lighting ? character_colored_item_light(p) : std::optional<cpu_colored_item_light>{};
    if (colored_light) {
...
auto map::direct_sunlight_state_at(const point_bub_ms p, const int zlev) const
    -> direct_sunlight_state {
    if (zlev >= OVERMAP_HEIGHT) { return direct_sunlight_state::direct; }
    ...
    for (const auto step : std::views::iota(1, levels_up + 1)) {
        ...
        if (has_vertical_light_blocker(above, p.x(), p.y())) { return direct_sunlight_state::none; }
    }
    if (!angled_sunlight) { return direct_sunlight_state::direct; }
    ...
    for (const auto step : std::views::iota(1, levels_up + 1)) {
        ...
        if (above.floor_cache[above.idx(sx, sy)]) { return direct_sunlight_state::shadow; }
    }
    ...
    const auto total = std::max(
        {std::abs(dx_to_sky), std::abs(dy_to_sky), static_cast<float>(levels_up)});
    ...
    for (const auto i : std::views::iota(1, steps + 1)) {
        ...
        const auto sz = std::clamp(
            zlev + static_cast<int>(std::lround(static_cast<float>(levels_up) * t)), zlev,
            OVERMAP_HEIGHT);
        if (sx < 0 || sy < 0) { return direct_sunlight_state::direct; }
        const auto& ray_cache = get_cache_ref(sz);
        if (sx >= ray_cache.cache_x || sy >= ray_cache.cache_y) {
            return direct_sunlight_state::direct;
        }
        if (sx == p.x() && sy == p.y() && sz == zlev) { continue; }
...
void map::build_sunlight_cache(int pzlev) {
    ...
    const int zlev_max =
        clamp(calc_max_populated_zlev() + 1, std::min(OVERMAP_HEIGHT, pzlev + 1), OVERMAP_HEIGHT);

    // true if all previous z-levels are fully transparent to light (no floors, transparency >= air)
    bool fully_outside = true;

    // true if no light reaches this level, i.e. there were no lit tiles on the above level (light
    // level <= inside_light_level)
    ...
    for (int zlev = zlev_max; zlev >= -OVERMAP_DEPTH; zlev--) {
        ...
        // TODO: if zlev < 0 is open to sunlight, this won't calculate correct light, but neither
        // does g->natural_light_level()
        ...
        // If there were no obstacles before this level, just app.y() weather illumination since
        // there's no opportunity for light to be blocked.
        if (fully_outside) {
            // No floors above: every tile has unobstructed sky, so all get the natural outdoor
            // level.
            ...
            for (int x = 0; x < map_cache.cache_x; ++x) {
                for (int y = 0; y < map_cache.cache_y; ++y) {
                    ...
                    // && semantics below is important, we want to skip the evaluation if possible,
                    // do not replace with &=

                    // fully_outside stays true if tile is transparent and there is no floor
                    fully_outside =
                        fully_outside
                        && this_transparency_cache[map_cache.idx(x, y)]
        ...
        for (int x = 0; x < map_cache.cache_x; ++x) {
            for (int y = 0; y < map_cache.cache_y; ++y) {
                ...
                for (int i = 0; i < 5; ++i) {
                    ...
                    bool inbounds =
                        prev_x >= 0 && prev_x < prev_map_cache.cache_x && prev_y >= 0
                        && prev_y < prev_map_cache.cache_y;

                    if (!inbounds) { continue; }

                    float prev_light_max;
                    float prev_transparency =
                        prev_transparency_cache[prev_map_cache.idx(prev_x, prev_y)];
                    // This is pretty gross, this cancels out the per-tile transparency effect
                    // derived from weather.
                    if (outside_cache[map_cache.idx(x, y)]) { prev_transparency /= sight_penalty; }
                    ...
                    if (prev_transparency > LIGHT_TRANSPARENCY_SOLID
                        ...
                        const float light_level =
                            clamp(prev_light_max * LIGHT_TRANSPARENCY_OPEN_AIR / prev_transparency,
                        ...
                        if (i == 0) {
                            ...
                            lm[map_cache.idx(x, y)] =
                                std::max(lm[map_cache.idx(x, y)], light_level);
                        }
                    }
                }
            }
        ...
        if (angled_sunlight_shadows && m_solar.direct_active) {
            ...
            std::ranges::
                for_each(std::views::iota(0, map_cache.cache_x * map_cache.cache_y), [&](int i) {
...
void map::generate_lightmap(const int zlev) {
    ...
    for (monster& critter : g->all_monsters()) {
        ...
        if (inbounds(mp)) {
            ...
            if (critter.type->luminance > 0) { apply_light_source(mp, critter.type->luminance); }
        }
    }
...
void map::generate_lightmap_worker(const int zlev) {
    ...
    */
    auto& light_source_buffer = map_cache.light_source_buffer;
    auto add_deferred_point_light =
        [&](const tripoint_bub_ms& source, const float luminance, const uint32_t color_rgb = 0u) {
    ...
    std::vector<std::pair<tripoint_bub_ms, float>> lm_override;
    {
            ...
            for (int smy = 0; smy < my_MAPSIZE; ++smy) {
                ...
                for (const auto sm_ms : submap_tiles()) {
                    ...
                    auto has_floor_above = [&](int idx) {
                        return prev_map_cache.floor_cache[idx]
                            || prev_map_cache.vehicle_floor_cache[idx];
                    };
                    ...
                    auto inbounds_light_neighbour = [&](const auto& neighbour) {
                        if (neighbour.x() < 0 || neighbour.y() < 0
                            || neighbour.x() >= map_cache.cache_x
                            || neighbour.y() >= map_cache.cache_y) {
                            return false;
                        }
                        return true;
                    };
                    auto direct_light_neighbour = [&](const auto& neighbour) {
                        return inbounds_light_neighbour(neighbour) && direct_sky(neighbour);
                    };
                    ...
                    if (!outside_cache[cur_idx] || (!top_floor && has_floor_above(cur_idx))) {
                        ...
                        for (int i = 0; i < 4; ++i) {
                            ...
                            if (!direct_light_neighbour(neighbour)) { continue; }
                            const bool nb_has_open_sky_neighbour =
                                std::ranges::any_of(std::views::iota(0, 4), [&](int j) {
                                    const auto cn = neighbour + point(dir_x[j], dir_y[j]);
                                    return cn != p.xy() && open_sky_neighbour(cn);
                                });
                            if (!nb_has_open_sky_neighbour) { continue; }
                            const float source_light = std::
                                min(natural_light, lm[map_cache.idx(neighbour.x(), neighbour.y())]);
                    ...
                    if (terrain->light_emitted > 0) {
                        add_deferred_point_light(
                            p, terrain->light_emitted,
                    ...
                    std::ranges::for_each(cur_submap->get_field(sm_ms), [&](auto& fld) {
                        if (!fld.first.is_valid()) {
                            debugmsg(
                                "generate_lightmap: invalid field type id %d at "
                                "grid(%d,%d,%d) tile(%d,%d) field_count=%d is_uniform=%d",
                                fld.first.to_i(), smx, smy, zlev, sm_ms.x(), sm_ms.y(),
                                cur_submap->field_count, static_cast<int>(cur_submap->is_uniform));
                            return;
                        }
                        ...
                        if (light_emitted > 0) {
                            add_deferred_point_light(
                                p, light_emitted, field_light_color(*cur).value_or(0u));
                        }
        ...
        std::ranges::for_each(smx_accs, [&](auto& local) {
            lm_override
                .insert(lm_override.end(), local.lm_override.begin(), local.lm_override.end());
            ...
            for (const auto& part : lights) {
                ...
                if (vp.has_flag(VPFLAG_CONE_LIGHT)) {
                    ...
                    const auto direction =
                        rotating_light.direction_at(base_direction, calendar::turn);
                    for (const auto beam_index : std::views::iota(0, rotating_light.beam_count())) {
                        const auto beam_direction =
                            direction
                            + rotating_light.beam_spacing() * static_cast<double>(beam_index);
                        apply_vehicle_arc({
                            .src = src,
            ...
            for (const vpart_reference& vp : v->get_all_parts()) {
                ...
                if (vp.has_feature(VPFLAG_CARGO) && !vp.has_feature("COVERED")) {
                    add_light_from_items(
                        pp, v->get_items(static_cast<int>(p)).begin(),
    ...
    */
    {
        ...
        for (const auto& source : map_cache.light_source_points) {
            ...
            if (luminance <= 0.0f && colored_luminance <= 0.0f) { continue; }
            if (color_rgb != 0u && colored_luminance >= luminance) {
                apply_light_source(p, colored_luminance, color_rgb);
            } else {
                if (luminance > 0.0f) { apply_light_source(p, luminance); }
...
auto map::light_at(const tripoint_bub_ms& p) const -> lit_level {
    ...
    record_cpu_lm_read(
        map_cache.lm_cpu_cache_valid, cpu_lm_light_reads_valid, cpu_lm_light_reads_stale);
    const auto& lm = map_cache.lm;
    const auto& sm = map_cache.sm;
    if (sm[map_cache.idx(p.x(), p.y())] >= LIGHT_SOURCE_BRIGHT) { return lit_level::BRIGHT; }

    const float max_light = lm[map_cache.idx(p.x(), p.y())];
    if (max_light >= LIGHT_AMBIENT_LIT) { return lit_level::LIT; }

    if (max_light >= LIGHT_AMBIENT_LOW) { return lit_level::LOW; }
...
auto map::ambient_light_at(const tripoint_bub_ms& p, const std::source_location location) const
    ...
    record_cpu_lm_read(
        map_cache.lm_cpu_cache_valid, cpu_lm_ambient_reads_valid, cpu_lm_ambient_reads_stale);
...
static auto scaled_visibility_for_view_distance(
    const float vis, const float visibility_scale_factor) -> float {
    ...
    if (vis <= LIGHT_TRANSPARENCY_SOLID) { return 0.0f; }
    if (vis >= VISIBILITY_FULL) { return VISIBILITY_FULL; }
    if (visibility_scale_factor == 1.0f) { return vis; }
...
static auto perception_visibility_range_base(const Character& viewer) -> float {
    ...
    return baseline_range
         + static_cast<float>(viewer.get_per() - perception_baseline)
...
auto map::make_visibility_variables(const int zlev) const -> visibility_variables {
    ...
    variables.visibility_range = std::max(
        variables.detail_range, scaled_visibility_range(perception_visibility_range_base(g->u)));
    variables.visibility_scale_factor = visibility_scale_factor_from_range(
        variables.visibility_range);
    return variables;
}
...
auto map::apparent_light_helper(
    ...
    if (p_opaque && scaled_vis > 0) {
        ...
        for (const point& offset : adjacent_offsets) {
            ...
            if (neighbour.x() < 0 || neighbour.y() < 0 || neighbour.x() >= map_cache.cache_x
                || neighbour.y() >= map_cache.cache_y) {
                continue;
            }
            ...
            visible_surface_light = std::max(
                visible_surface_light, map_cache.lm[map_cache.idx(neighbour.x(), neighbour.y())]);
        }
...
auto map::apparent_light_at(
...
#endif
    const apparent_light_info a =
        apparent_light_helper(map_cache, p, cache.visibility_scale_factor);
    ...
    if (apparent_light > LIGHT_SOURCE_BRIGHT || source_light) { return lit_level::BRIGHT; }
    if (apparent_light > LIGHT_AMBIENT_LIT) { return lit_level::LIT; }
...
// Alters the vision caches to the player specific version, the restore caches will be filled so it
// can be undone with restore_vision_transparency_cache
void map::apply_vision_transparency_cache(
    ...
    for (point adjacent : eight_adjacent_offsets) {
        ...
        if (vision_transparency_cache[i] == VISION_ADJUST_SOLID) {
            ...
            if (std::ranges::find(four_diagonal_offsets, adjacent) == four_diagonal_offsets.end()) {
            ...
            bool& relevant_blocked =
                ...
                : adjacent == point_south_west
                    ? blocked_data[p.x() * sy + p.y()].ne
                    :
...
void map::restore_vision_transparency_cache(
    const tripoint_bub_ms& center, int target_z, float (&vision_restore_cache)[9],
    bool (&blocked_restore_cache)[8]) {
...
void map::build_seen_cache(const tripoint_bub_ms& origin, const int target_z) {
...

    {
        ...
        for (int z = -OVERMAP_DEPTH; z <= OVERMAP_HEIGHT; z++) {
            ...
            transparency_caches[idx] =
                {cur_cache.transparency_cache.data(), cur_cache.cache_x, cur_cache.cache_y};
            seen_caches[idx] = {cur_cache.seen_cache.data(), cur_cache.cache_x, cur_cache.cache_y};
            floor_caches[idx] =
                {cur_cache.floor_cache.data(), cur_cache.cache_x, cur_cache.cache_y};
            vehicle_floor_caches[idx] =
                {cur_cache.vehicle_floor_cache.data(), cur_cache.cache_x, cur_cache.cache_y};
            blocked_caches[idx] =
        ...
        // Accumulated cumulatively so each z-level costs one OR-sweep instead of k.
        {
                    ...
                    if (cx >= 0 && cy >= 0) {
                        if (oz != cz && oz > -OVERMAP_DEPTH && cz < OVERMAP_HEIGHT) {
                            if (oz < cz) {
                                ...
                                if (cx < fc.sx && cy < fc.sy && fc.at(cx, cy)) { return false; }
                            } else {
                        ...
                        if (cx < ic.sx && cy < ic.sy && ic.at(cx, cy) <= LIGHT_TRANSPARENCY_SOLID) {
                            return false;
                        }
                    }
                    ox = cx;
            ...
            if (use_3d_shadowcasting) { temp_seen.resize(cache_sz); }
            ...
            const auto process_z_level = [&](int z) {
                    ...
                    for (int y = 0; y < zc.cache_y; ++y) {
                        ...
                        if (sc > 0.0f) {
                            ...
                            if (temp_seen[tile_idx] > 0.0f) {
                                ...
                                if (floor_crossing_blocked(x, y, z)) { sc = 0.0f; }
                                continue;
                            }
                ...
                if (!use_3d_shadowcasting) {
                    ...
                    for (int x = 0; x < zc.cache_x; ++x) {
                        for (int y = 0; y < zc.cache_y; ++y) {
                            const int tile_idx = zc.idx(x, y);
                            if (zc.seen_cache[tile_idx] > 0.0f) { continue; }
                            const float best = std::ranges::fold_left(
                                k_dirs, 0.0f,
                                [&](float acc, const std::pair<int, int>& d) -> float {
                                    const int nx = x + d.first;
                                    const int ny = y + d.second;
                                    if (nx < 0 || ny < 0 || nx >= zc.cache_x || ny >= zc.cache_y) {
                                        return acc;
                                    }
                            ...
                            if (best > 0.0f) { zc.seen_cache[tile_idx] = best; }
                        }
                    }
                }
            };
            ...
            for (int z = origin.z() - 1; z >= z_lo; --z) {
                ...
                std::ranges::
                    transform(sc_span, vfc_span, sc_span.begin(), [](float s, bool v) -> float {
                        return v ? 0.0f : s;
                    });
            }
    ...
    for (const int mirror : mirrors) {
        ...
        if (is_camera) {
            const auto raw_range =
                veh->part_info(mirror).bonus * veh->part(mirror).hp()
...
static const light_model k_light_model =
    {light_calc, light_check, update_light, nullptr, light_from_lookup, accumulate_transparency};

namespace {

auto apply_cpu_colored_light_3d(map& here, const cpu_colored_light_opt& opt) -> void {
    if (!colored_lighting || opt.color_rgb == 0u || opt.luminance <= LIGHT_AMBIENT_LOW
        || !here.inbounds(opt.source)) {
        return;
    }
    const auto adjacent_z_intensity =
        opt.luminance / (std::exp(LIGHT_TRANSPARENCY_OPEN_AIR * Z_LEVEL_SCALE) * Z_LEVEL_SCALE);
    if (adjacent_z_intensity <= LIGHT_AMBIENT_LOW) { return; }
    ...
    cast_zlight(
        output_caches, transparency_caches, floor_caches, blocked_caches, opt.source, 0,
...
void map::apply_light_source(const tripoint_bub_ms& p, float luminance, uint32_t color_rgb) {
    ...
    */
    ...
    auto mask = uint8_t{};
    if (north) { mask |= OCTANT_NORTH; }
    if (east) { mask |= OCTANT_EAST; }
    if (south) { mask |= OCTANT_SOUTH; }
    if (west) { mask |= OCTANT_WEST; }
    if (mask != 0) {
        auto colored_context = cpu_colored_light_cache_context{};
        castLightOctants(
            lm_data, trans_data, blocked_data, sx, sy, p2, 0, luminance, k_light_model, mask,
            &weather_lookup_, make_colored_light_callback(cache, color_rgb, colored_context));
    }
    if (color_rgb != 0u) {
        apply_cpu_colored_light_3d(
            *this,
            {
                .source = p,
                .luminance = luminance,
...
auto map::apply_directional_light(const apply_directional_light_options& opt) -> void {
    ...
    if (mask != 0) {
        auto colored_context = cpu_colored_light_cache_context{};
        castLightOctants(
            lm_data, trans_data, blocked_data, sx, sy, p2, 0, opt.luminance, k_light_model, mask,
            &weather_lookup_, make_colored_light_callback(cache, opt.color_rgb, colored_context));
    }
}

void map::apply_light_arc(
    const tripoint_bub_ms& p, units::angle angle, float luminance, units::angle wideangle) {
    apply_light_arc({
        .p = p,
        .angle = angle,
        .luminance = luminance,
...
auto map::apply_light_arc(const apply_light_arc_options& opt) -> void {
    if (opt.luminance <= LIGHT_SOURCE_LOCAL) { return; }

    const auto& arc_cache = get_cache(opt.p.z());
    auto lit = std::vector<bool>(static_cast<size_t>(arc_cache.cache_x) * arc_cache.cache_y, false);
...
void map::apply_light_ray(
    std::vector<bool>& lit, const tripoint_bub_ms& s, const tripoint_bub_ms& e, float luminance) {
    apply_light_ray({
        .lit = lit,
        .s = s,
        .e = e,
...
auto map::apply_light_ray(const apply_light_ray_options& opt) -> void {
    auto a = point_bub_ms(std::abs(opt.e.x() - opt.s.x()) * 2, std::abs(opt.e.y() - opt.s.y()) * 2);
    auto d = point_bub_ms((opt.s.x() < opt.e.x()) ? 1 : -1, (opt.s.y() < opt.e.y()) ? 1 : -1);
    auto p = opt.s.xy();

    // TODO: Invert that z comparison when it's sane
    if (opt.s.z() != opt.e.z() || (opt.s.x() == opt.e.x() && opt.s.y() == opt.e.y())) { return; }
    ...
    const float scaling_factor =
        static_cast<float>(rl_dist(opt.s, opt.e)) / static_cast<float>(square_dist(opt.s, opt.e));
