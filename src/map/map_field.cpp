...
static inline auto check_flammable(const map_data_common_t& t) -> bool { return t.is_flammable(); }
...
void map::player_in_field(player& u) {
    ...
    for (auto& field_list_it : curfield) {
        ...
        if (ft == fd_sludge) {
            ...
            if (!u.in_vehicle) {
                u.add_msg_if_player(
                    m_bad, _("The sludge is thick and sticky.  You struggle to pull free."));
...
void map::spill_liquid_field(const tripoint_bub_ms& center, const item& liquid) {
    ...
    const auto spill_tiles = divide_round_up(
        units::to_milliliter(liquid.volume()), units::to_milliliter(spill_tile_volume));
    ...
    const auto sort_spill_tiles = [&]() {
        ranges::
            sort(spill_tiles_to_fill, [&](const tripoint_bub_ms& lhs, const tripoint_bub_ms& rhs) {
    ...
    const auto add_candidate =
        [&](const tripoint_bub_ms& current, const tripoint_bub_ms& adjacent) {
    ...
    const auto expand_candidates = [&](const tripoint_bub_ms& current) {
        for (const auto& [dx, dy] : spilled_liquid_adjacent_offsets) {
            ...
            if (get_field(adjacent, type) != nullptr) { continue; }
    ...
    if (get_field(center, type) != nullptr) {
        ...
        while (!frontier.empty()) {
            ...
            if (existing_field == nullptr) { continue; }
            ...
            for (const auto& [dx, dy] : spilled_liquid_adjacent_offsets) {
                ...
                if (get_field(adjacent, type) != nullptr) {
                    if (!visited.contains(adjacent)) { frontier.push(adjacent); }
                    continue;
                }
                add_candidate(current, adjacent);
            }
    ...
    while (represented_amount < target_amount) {
        ...
        for (const auto& tile : spill_tiles_to_fill) {
            ...
            if (existing_field == nullptr
                || existing_field->get_field_intensity() >= max_field_intensity) {
                continue;
            }
            mod_field_intensity(tile, type, 1);
            represented_amount++;
            changed = true;
            if (represented_amount >= target_amount) { break; }
        }
        if (represented_amount >= target_amount) { break; }
...
auto mark_field_cache_dirty(
    field_cache_dirty_context const& ctx, const tripoint_abs_sm& abs_sm, const field_type_id& type)
    -> void {
    if (!type) { return; }
    const auto& data = type.obj();
    if (!data.dirty_transparency_cache && data.is_transparent()) { return; }
    if (!submap_loader.is_properly_requested(ctx.dimension, abs_sm)) { return; }
...
auto sub_remove_field(field_cache_dirty_context const& ctx, SubTile& dst, const field_type_id& type)
    -> bool {
    if (!dst.valid() || !dst.get_field().remove_field(type)) { return false; }
    --dst.sm->field_count;
    dst.sm->is_uniform = false;
...
auto consume_fire_smothering_puddle(field& curfield, const field_type_str_id& field_type) -> bool {
    auto* liquid = curfield.find_field(field_type.id());
    if (liquid == nullptr) { return false; }
    if (liquid->get_field_intensity() < liquid->get_max_field_intensity()) { return false; }
...
auto gas_spread_sub(field_cache_dirty_context const& dirty, field_entry& cur, SubTile& dst)
    -> void {
...
auto process_fields_in_submap(
    ...
    std::ranges::for_each(field_positions, [&](const point_sm_ms& local) {
        ...
        for (auto it = curfield.begin(); it != curfield.end();) {
                ...
                auto time_added = 0_turns;
                    ...
                    if (!is_sealed && !items_here.empty()) {
                        ...
                        while (fuel_it != items_here.end() && consumed < max_c) {
                            ...
                            if (destroyed) {
                                std::ranges::for_each(
                                    fuel->contents.clear_items(), [&](detached_ptr<item>& ci) {
                    ...
                    if (can_burn) {
                        ...
                        } else if ((ter.is_hard_flammable() || frn.is_hard_flammable())
                                   && one_in(3)) {
                    ...
                    if (can_spread && pos.z() < OVERMAP_HEIGHT && cur.get_field_intensity() == 3) {
                        ...
                        if (above_sm) {
                            ...
                            if (above_ter.has_flag(TFLAG_NO_FLOOR) || above_ter.is_flammable()) {
                                auto* fire_above = above_sm->get_field(local).find_field(fd_fire);
                                if (fire_above) {
                                    fire_above->mod_field_age(-2_turns);
                                } else if (
                                    above_sm->get_field(local).add_field(fd_fire, 1, 0_turns)) {
                    ...
                    if (can_spread) {
                        std::ranges::for_each(eight_dirs_sm, [&](const point& d) {
                            ...
                            if (in_pit != (dst.get_ter_t().id.id() == t_pit)) { return; }
                        ...
                        std::ranges::for_each(std::views::iota(0u, 8u), [&](size_t c) {
                            ...
                            const auto can_ignite =
                                ...
                                && ((power >= 3 && cur.get_field_age() < 0_turns && one_in(20))
                                    || (power >= 2
                                        && (check_basic_flammable(dter)
                                            || check_basic_flammable(dfur))
                                        && one_in(2))
                                    || (power >= 2
                                        && (check_ash_flammable(dter) || check_ash_flammable(dfur))
                                        && one_in(2))
                                    || (power >= 3
                                        && (check_hard_flammable(dter) || check_hard_flammable(dfur))
                                        && one_in(5))
            ...
            cur.set_field_age(
                cur.get_field_age() + action_time_scale::calendar_duration_this_tick());
            ...
            if (!cur.is_field_alive()) {
                ...
                dirty_transparency_cache =
                    dirty_transparency_cache || removed_fd_type.dirty_transparency_cache
