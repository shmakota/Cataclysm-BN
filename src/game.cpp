...
auto avatar_grabbed_creature() -> Creature *
{
    ...
    for( const tripoint_bub_ms &p : get_map().points_in_radius( you.bub_pos(), 1, 0 ) ) {
...
auto avatar_remembers_stairs_at( const avatar &you, map &mp, const tripoint_bub_ms &candidate,
                                 const bool going_up, const bool going_down ) -> bool
{
    const auto abs_candidate = mp.bub_to_abs( candidate );
