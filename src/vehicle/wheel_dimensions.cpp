...
auto parse_metric_dimension(const JsonObject& jo, const std::string& member) -> int {
    ...
    if (value < 0) { jo.throw_error("wheel dimensions must not be negative", member); }

    if (suffix != "mm") { jo.throw_error("wheel dimensions must use the mm suffix", member); }
...
auto read_from_json(const JsonObject& jo, const std::string& member) -> std::optional<int> {
    if (!jo.has_member(member)) { return std::nullopt; }

    if (jo.has_int(member)) { return from_legacy_inches(jo.get_int(member)); }

    if (jo.has_string(member)) { return parse_metric_dimension(jo, member); }

    jo.throw_error(
        "wheel dimensions must be an integer legacy inch value or a \"<n> mm\" string", member);
}

auto format_for_display(const int millimeters) -> std::string {
    if (get_option<std::string>("DISTANCE_UNITS") == "imperial") {
        const auto inches = static_cast<int>(
            std::lround(static_cast<double>(millimeters) / legacy_millimeters_per_inch));
