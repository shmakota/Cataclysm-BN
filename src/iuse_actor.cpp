...
void foraging_item_iuse::load( const JsonObject &obj )
{
    ...
    obj.read( "completion_message", completion_message );
    obj.read( "invalid_message", invalid_message );
    obj.read( "moves", moves );
    obj.read( "need_charges_,msg", need_charges_msg );
    obj.read( "need_charges", need_charges );
    need_charges = std::max( need_charges, 0 );
}

int foraging_item_iuse::use( player &p, item &it, bool, const tripoint & ) const
{
    map &here = get_map();
    ...
    if( !here.passable( target_pos.value() ) || !valid_tile ) {
        p.add_msg_if_player( m_bad, _( invalid_message ) );
        failed = true;
    }
    ...
    if( one_in( one_in_amount ) ) {
        ...
        p.add_msg_if_player( m_info, _( completion_message ) );
    } else {
